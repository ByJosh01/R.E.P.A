...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const showInfoModal = (title, content, isSuccess = true, onConfirm = null) => {
        ...
        // Permitimos HTML directo
        adminInfoContent.innerHTML = content.startsWith('<div') ? content : `<p style="text-align: center;">${content}</p>`;
    ...
    // ▼▼▼ LÓGICA CORREGIDA DEL MODAL DE INFORMACIÓN ▼▼▼
    if (viewAdminInfoBtn) {
        viewAdminInfoBtn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                const nombreCompletoAdmin = [p.nombre, p.apellido_paterno, p.apellido_materno]
                    .filter(part => part && part.trim() !== '')
                    .join(' ') || 'N/A';
                
                // HTML completo con todos los campos
                const infoHtml = `
                    <div class="info-row"><label>Nombre:</label> <span>${nombreCompletoAdmin}</span></div>
                ...
                showInfoModal('Información del Superadministrador', infoHtml, true);
    ...
    const renderTabla = (solicitantes) => {
        ...
        if (solicitantes.length === 0) { tableBody.innerHTML = `<tr><td colspan="6" style="text-align: center;">No se encontraron solicitantes para el criterio seleccionado.</td></tr>`; return; }
        ...
        tableBody.addEventListener('click', async (e) => {
            ...
            if (button) {
                ...
                // Delete
                    ...
                    if (deleteConfirmMessage) deleteConfirmMessage.innerHTML = `¿Estás seguro de que deseas eliminar a <strong>${nombre}</strong>? Esta acción no se puede deshacer.`;
    ...
    if (btnDownloadGeneralReport) {
        btnDownloadGeneralReport.addEventListener('click', async () => {
            const originalText = btnDownloadGeneralReport.innerHTML;
            ...
            try {
                ...
                const a = document.createElement('a'); a.style.display = 'none'; a.href = urlBlob; a.download = filename;
                document.body.appendChild(a); a.click();
                window.URL.revokeObjectURL(urlBlob); a.remove();
            } catch (error) {
                ...
                btnDownloadGeneralReport.innerHTML = originalText;
            }
        });
    }
