...
            // Consulta SQL corregida (limpia)
            const [rows] = await pool.query(
                `SELECT u.id, u.curp, u.email, u.rol, s.solicitante_id FROM usuarios u LEFT JOIN solicitantes s ON u.id = s.usuario_id WHERE u.id = ?`,
                [decoded.id]
            );
            
            if (!rows[0]) {
...
// Middleware NUEVO (bueno, de vuelta) para Superadmin SOLAMENTE (Acciones peligrosas)
